/*
 * Derive v3 API
 *
 * JSON-RPC 2.0 methods, served over WebSocket and HTTP POST.
 *
 * The version of the OpenAPI document: 0.2.0
 * 
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize)]
pub struct PublicExecuteQuoteDebugRequest {
    #[serde(rename = "direction")]
    pub direction: models::Direction,
    #[serde(rename = "legs")]
    pub legs: Vec<models::PricedLegParamsAndResponse>,
    /// Non-negative decimal string of the human value (e.g. `\"1.5\"`), up to 12 fractional digits; a string or JSON number is accepted
    #[serde(rename = "max_fee")]
    pub max_fee: bigdecimal::BigDecimal,
    #[serde(rename = "nonce")]
    pub nonce: i64,
    /// UUID v4 string
    #[serde(rename = "quote_id")]
    pub quote_id: uuid::Uuid,
    /// UUID v4 string
    #[serde(rename = "rfq_id")]
    pub rfq_id: uuid::Uuid,
    #[serde(rename = "signature")]
    pub signature: String,
    #[serde(rename = "signature_expiry_sec")]
    pub signature_expiry_sec: i64,
    /// 20-byte Ethereum address as a 0x-prefixed lowercase hex string.
    #[serde(rename = "signer")]
    pub signer: String,
    #[serde(rename = "subaccount_id")]
    pub subaccount_id: i32,
}

impl PublicExecuteQuoteDebugRequest {
    pub fn new(direction: models::Direction, legs: Vec<models::PricedLegParamsAndResponse>, max_fee: bigdecimal::BigDecimal, nonce: i64, quote_id: uuid::Uuid, rfq_id: uuid::Uuid, signature: String, signature_expiry_sec: i64, signer: String, subaccount_id: i32) -> PublicExecuteQuoteDebugRequest {
        PublicExecuteQuoteDebugRequest {
            direction,
            legs,
            max_fee,
            nonce,
            quote_id,
            rfq_id,
            signature,
            signature_expiry_sec,
            signer,
            subaccount_id,
        }
    }
}

